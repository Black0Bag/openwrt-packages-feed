...
// Settings map to memos CLI flags (cmd/memos/main.go) via init.d argv assembly.
...
return view.extend({
	render: function () {
		var map = new form.Map('memos', _('Memos - Settings'),
			_('Settings are passed to the memos binary as command-line flags by the init script. Some options (driver, mirror, release channel) only affect how the package downloads or runs memos and are not memos CLI flags themselves.'));
		...
		var oData = s.taboption('general', form.Value, 'data', _('Data directory'),
			_('Where memos stores its SQLite database and uploaded attachments. Must be on a writable, persistent filesystem; data on a tmpfs will be lost on reboot.'));
		...
		oDriver.value('sqlite',  _('SQLite (embedded, recommended)'));
		oDriver.value('mysql',   _('MySQL / MariaDB'));
		oDriver.value('postgres', _('PostgreSQL'));
		oDriver.default = 'sqlite';

		var oDsn = s.taboption('database', form.Value, 'dsn', _('DSN'),
			_('Database source name for the chosen driver. Required when driver is not SQLite. Example (MySQL): user:pass@tcp(127.0.0.1:3306)/memos?parseTime=true Example (Postgres): postgres://user:pass@127.0.0.1:5432/memos?sslmode=disable'));
		...
		var oMirror = s.taboption('download', form.ListValue, 'mirror', _('Download mirror'),
			_('When GitHub is not reachable from your region, pick a mirror prefix to proxy binary download URLs. The GitHub API is always accessed directly. Mirror availability is not guaranteed; switch to "Custom" if the built-in ones stop working.'));
		...
		var oCustom = s.taboption('download', form.Value, 'mirror_custom', _('Custom mirror prefix'),
			_('A prefix prepended to github.com URLs. Must end with "/". Leave empty to disable the custom prefix.'));
		...
		var oChannel = s.taboption('download', form.ListValue, 'release_channel', _('Release channel'),
			_('Stable points at the GitHub releases/latest endpoint (recommended). Beta pulls the newest release (including pre-releases) for those who want to test.'));
		...
		var oCron = s.taboption('download', form.Flag, 'cron_autoupdate', _('Daily auto-update'),
			_('Install a daily 3:30 AM cron job that runs memos.sh update. Only the memos binary is replaced; your data is untouched.'));
		oCron.default = '0';
		oCron.rmempty = false;

		var oInstance = s.taboption('advanced', form.Value, 'instance_url', _('Instance URL'),
			_('Public URL of your memos instance, e.g. https://memos.example.com. Setting this enables the "public" access mode (anonymous read access).'));
